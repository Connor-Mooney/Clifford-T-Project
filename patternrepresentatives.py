...
def tupadd(t1, t2):
    ...
    if t1[2] <= t2[2]:
        ...
        t3 = (t1cop[0] + t2cop[0], t1cop[1] + t2cop[1], t2cop[2])
    while t3[0]%2 == 0 and (t3[0] != 0 and t3[1] != 0):
        t3 = (t3[1], int(t3[0]/2), t3[2]-1)
    return t3
...
def Tmat(i, j):
    ...
    T = zeroMat
    T[i-1][j-1] = (-1,0,1)
    T[i-1][i-1], T[j-1][i-1], T[j-1][j-1] = (1,0,1), (1,0,1), (1,0,1)
    for k in range(0,6):
        if k != i-1 and k != j-1:
            T[k][k] = (1,0,0)
    return(T)
...
for i in range(len(Ts)):
    tcountpatterns[0].append((Ts[i], 1, residue(Ts[i]), Tlabels[i]))
        ...
        for tup in tcountpatterns[j-1]:
            ...
            if colpermcheck(matmult(tup[0],Ts[i])) not in allmats:
                ...
                if not CliffCheck(matmult(tup[0],Ts[i])):
                    tcountpatterns[j].append((matmult(tup[0],Ts[i]), j+1, residue(matmult(tup[0],Ts[i])), tup[3]+Tlabels[i]))
                    allmats.append(colpermcheck(matmult(tup[0],Ts[i])) )

for row in tcountpatterns:
    print(len(row))

##print(matmult(Tmat(1,2),Tmat(3,4)))
##print(matmult(Tmat(3,4),Tmat(1,2)))
##
##print(matmult(Tmat(1,1),Tmat(1,1)))
